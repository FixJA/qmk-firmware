...
#define rowC_bits (PAL_PORT_BIT(PAL_PAD(C15)))
...
// matrix code
// ultra fast read_cols code // Halo65v2
...
static inline void select_row(uint8_t row) {
    if (row > 0)
        palClearPort(PAL_PORT(A0), PAL_PORT_BIT(row - 1));
    else
        palClearPort(PAL_PORT(C0), PAL_PORT_BIT(15));
}
